...
export const overviewService = {
  async getOverview(accountId: string): Promise<OverviewDTO> {
    const cacheKey = cacheService.buildKey(accountId, 'overview');
    const cached = await cacheService.get<OverviewDTO>(cacheKey, accountId);
    if (cached) {
      return cached;
    }
    ...
    const result: OverviewDTO = {
      ...
      recentProjects: recentResult.rows.map((r) => ({
        id: r.id,
        name: r.name,
    ...
    await cacheService.set(cacheKey, result, OVERVIEW_TTL, accountId);
    return result;
  },
};
