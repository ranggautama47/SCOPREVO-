...
const ALLOWED_MIME_TYPES = ['application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'text/markdown'];
...
export async function uploadDocumentBuffer(
  ...
  if (buffer.byteLength > maxSize) {
    throw new Error(`File size exceeds 2 MB limit (got ${buffer.byteLength} bytes)`);
  }
...
export async function verifyProjectOwnership(projectId: string, accountId: string): Promise<void> {
