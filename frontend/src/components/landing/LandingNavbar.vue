  ...
  >
    ...
    >
      ...
      >
        ...
        />
        <span
          class="font-['Baskervville',serif] text-xl md:text-2xl tracking-tight text-[#1A1A1A]"
          >SCOPREVO</span
        >
      </router-link>
      ...
      <button
        ...
        class="lg:hidden p-2 border-2 border-[#1A1A1A] bg-[#FAFAF9] focus:ring-2 focus:ring-[#006D77] focus:outline-none"
        :aria-label="
    ...
    <Transition name="slide-down">
      <div
        v-if="menuOpen"
        class="absolute top-16 left-0 right-0 bg-[#FAFAF9] border-b-2 border-[#1A1A1A] flex flex-col p-4 gap-4 lg:hidden shadow-[0_4px_0px_0px_#1A1A1A] max-h-[60vh] overflow-y-auto"
      >
        <!-- Locale Toggle (mobile) - FIRST row inside menu panel -->
        <div
          role="group"
          :aria-label="t('landing.nav.languageGroup')"
          class="inline-flex w-fit items-center border-2 border-[#1A1A1A] rounded-none overflow-hidden"
        >
          <button
            v-for="loc in ['id', 'en'] as const"
            :key="loc"
            ...
            :aria-pressed="prefs.locale === loc"
            @click="prefs.setLocale(loc)"
            class="px-4 py-2.5 font-['JetBrains_Mono',monospace] text-[10px] font-bold uppercase tracking-wider border-r-2 border-[#1A1A1A] last:border-r-0 transition-colors cursor-pointer"
            :class="
              prefs.locale === loc
                ? 'bg-[#1A1A1A] text-[#FAFAF9]'
                : 'bg-[#FAFAF9] text-[#1A1A1A] hover:bg-[#FDFFB6]'
