...
<template>
  <section class="p-8 md:p-12">
    <div
      class="flex items-center justify-between border-b-2 border-[#1A1A1A] pb-6 mb-8"
    >
      <div>
        <h1
          class="font-['Baskervville',serif] text-5xl font-normal leading-[1.1] tracking-tight"
        >
          Projects
        </h1>
        <p
          class="font-['Noto Serif',serif] text-base leading-[1.6] text-[#1A1A1A]/60 mt-2"
        >
    ...
    <div
      v-if="isLoading"
      class="flex items-center gap-2 font-['JetBrains Mono',monospace] text-sm text-[#1A1A1A]/60"
    >
    ...
    <div
      v-else-if="error"
      class="bg-[#FEE2E2] text-[#991B1B] border-2 border-[#1A1A1A] p-6 rounded-none font-['Noto Serif',serif] mb-6"
    >
      {{ error }}
    </div>

    <!-- Empty State dengan Gambar -->
    <div
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      >
        <h3
          class="font-['Baskervville',serif] text-xl font-normal leading-[1.4]"
        >
          {{ project.name }}
        </h3>
        <p
          class="font-['Noto Serif',serif] text-sm leading-[1.5] text-[#1A1A1A]/60 mt-1"
        >
          {{ project.clientName }}
        </p>
        <div class="mt-4">
          <div class="flex items-center justify-between mb-1">
            <span
              class="font-['JetBrains Mono',monospace] text-xs text-[#1A1A1A]/60"
              >{{ project.usedRevisions }} /
              {{ project.totalAllowedRevisions }} revisions</span
            >
            <span
              class="font-['JetBrains Mono',monospace] text-xs text-[#1A1A1A]/60"
              >{{ project.remainingRevisions }} remaining</span
            >
          </div>
          <div
      ...
      >
        ...
        <form @submit.prevent="handleCreateProject" class="flex flex-col gap-4">
          <div>
            <label
              for="name"
              class="block font-['JetBrains Mono',monospace] text-xs uppercase tracking-wide mb-1"
              >Project Name</label
            >
            <input
              id="name"
          ...
          <div
            v-if="error"
            class="bg-[#FEE2E2] text-[#991B1B] border-2 border-[#1A1A1A] p-3 rounded-none font-['Noto Serif',serif] text-sm"
          >
            {{ error }}
          </div>
